<?php
/**
 * Belstroj AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Belstroj AB Leads" plugin, so leads survive a theme change.
 *
 * @package Belstroj AB
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'belstroj', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'belstroj' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'belstroj' ) ) );

		register_block_pattern_category( 'belstroj-sections', array( 'label' => __( 'Belstroj AB sections', 'belstroj' ) ) );
		register_block_pattern_category( 'belstroj-pages', array( 'label' => __( 'Belstroj AB page layouts', 'belstroj' ) ) );
	}
);
