...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'belstroj-leads' ),
					'singular_name' => __( 'Lead', 'belstroj-leads' ),
					'edit_item'     => __( 'Förfrågan', 'belstroj-leads' ),
					'search_items'  => __( 'Search leads', 'belstroj-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'belstroj-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'belstroj-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'belstroj-leads' ),
			'npl_phone'    => __( 'Phone', 'belstroj-leads' ),
			'npl_service'  => __( 'Needs', 'belstroj-leads' ),
			'npl_timeline' => __( 'When', 'belstroj-leads' ),
			'npl_zip'      => __( 'ZIP', 'belstroj-leads' ),
			'npl_source'   => __( 'Source', 'belstroj-leads' ),
			'npl_status'   => __( 'Status', 'belstroj-leads' ),
			'date'         => __( 'Received', 'belstroj-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'belstroj-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'belstroj-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'belstroj-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'belstroj-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'belstroj-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'belstroj-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'belstroj-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'belstroj-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'belstroj-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'belstroj-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'belstroj-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'belstroj-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
